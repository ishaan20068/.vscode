...
typedef priority_queue<int32_t>         pqi;
...
void merge(int* a,int low,int high){
    ...
    int i=low,j=mid+1,k=0;int final[high-low+1];
    while(i<mid+1 && j<high+1){
        if(a[i]<a[j]){final[k]=a[i];i++;k++;}
        else if(a[i]>a[j]){final[k]=a[j];j++;k++;}
        else{final[k]=a[i];k++;i++;final[k]=a[j];k++;j++;}
    }
    while(i<mid+1){final[k]=a[i];i++;k++;}
    while(j<high+1){final[k]=a[j];j++;k++;}
    fro(i,low,high+1){a[i]=final[i-low];}
}
