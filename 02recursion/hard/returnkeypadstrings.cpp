/*print all the strings that can be made by the button pressed on keypad
...
void f(string s,int index,string made){
    if(index==s.size()){o(made) return ;}
    fr(i,(ch[s[index]-48]).size()){string made1=made+(ch[s[index]-48])[i];f(s,index+1,made1);}
