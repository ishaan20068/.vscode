...
void ans(){
    node<int>* l=input();
    node<int>* oh=NULL;node<int>*eh=NULL;node<int>*ot=NULL;node<int>*et=NULL;
    while(l!=NULL){
        node<int>* k=l;
        l=l->next;
        k->next=NULL;
        if(k->data%2==1){if(oh==NULL){oh=k;ot=k;}else{ot->next=k;ot=k;}}
        else{if(eh==NULL){eh=k;et=k;}else{et->next=k;et=k;}}
    }
    ot->next=eh;
    oh->print();
}
