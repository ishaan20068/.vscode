...
void ans(){
    node<int>* h1=input();
    node<int>* h2=input();
    node<int>*temp=(h1->data>h2->data)?h2:h1;
    node<int>*t=temp;
    if(temp->data==h1->data){h1=h1->next;}else{h2=h2->next;}
    while(h1!=NULL && h2!=NULL){
        if(h1->data>h2->data){node<int>* k=new node<int>(h2->data);h2=h2->next;temp->next=k;temp=temp->next;}
        else{node<int>* k=new node<int>(h1->data);h1=h1->next;temp->next=k;temp=temp->next;}
    }
    if(h1==NULL){temp->next=h2;}else{temp->next=h1;}
    t->print();
}
int32_t main(){
    fast
