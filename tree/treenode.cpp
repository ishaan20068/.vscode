...
class tree{
    ...
    void addchild(t x){
        tree * m=new tree<t>(x);
        this->child.push_back(m);
        return ;
    }
    void delchild(tree<t> * x){
        for(int i=0;i<this->child.size();i++){
            if(this->child[i]->val==x->val){this->child.erase(this.child.begin()+i);}
        }
    }
    void delchild(t x){
        for(int i=0;i<this->child.size();i++){
            if(this->child[i]->val==x){this->child.erase(this->child.begin()+i);}
        }
    }
    void print(){
        cout<<this->val<<" ";
