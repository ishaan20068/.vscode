...
class tree{
    ...
    t val;
    vector<tree<t>*> child;
    tree(t num){this->val=num;}
    void addchild(t x){tree * m=new tree<t>(x);this->child.push_back(m);return ;}
    void delchild(tree<t> * x){for(int i=0;i<this->child.size();i++){if(this->child[i]->val==x->val){this->child.erase(this.child.begin()+i);}}}
    void delchild(t x){for(int i=0;i<this->child.size();i++){if(this->child[i]->val==x){this->child.erase(this->child.begin()+i);}}}
    void print(){cout<<this->val<<" ";for(auto i:this->child){i->print();}}
    int count(){int num=0;for(auto i:this->child){num+=i->count();}num++;return num;}
    int height(){int h=1;for(auto i:this->child){h=max(h,1+i->height());}return h;}
    int depth(tree<t> * x){int m1=this->height();int m2=x->height();return m1-m2;}
    int maxval(){int x=this->val;for(auto i:this->child){x=max(x,i->maxval());}return x;}
    int minval(){int x=this->val;for(auto i:this->child){x=min(x,i->minval());}return x;}
    ~tree(){for(auto i:this->child){delete i;}delete this;}
};


int nextbigger(tree<int>* t,int x){
    int num=-1;
    if(t->val>x){num=t->val;}
    for(auto i:t->child){
        if(num==-1){num=nextbigger(i,x);}
        else{if(nextbigger(i,x)!=-1 && num>nextbigger(i,x)){num=nextbigger(i,x);}}
    }
    return num;
}
